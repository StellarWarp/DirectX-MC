...
void SkyEffect::SetRenderTarget()
{
	static bool r_sun = false;
	r_sun = !r_sun;
	pImpl->m_pEffectHelper->GetConstantBufferVariable("g_renderSun")->SetUInt(r_sun);
}

void SkyEffect::SetSunColor(const XMFLOAT3& color)
{
	pImpl->m_pEffectHelper->GetConstantBufferVariable("g_SunColor")->SetFloatVector(3, (FLOAT*)&color.x);
}

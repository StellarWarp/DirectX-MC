...
void SkyRender::Draw(ID3D11DeviceContext* deviceContext, SkyEffect& skyEffect, const Camera& camera)
{
	...
	skyEffect.SetWorldMatrix(m_SkyTransform);
	...
	//太阳
	skyEffect.SetWorldMatrix(m_SunView);

	skyEffect.SetRenderTarget();
	skyEffect.Apply(deviceContext);
	deviceContext->DrawIndexed(m_IndexCount, 0, 0);
	skyEffect.SetRenderTarget();
}

void SkyRender::SetSkyRoate(const XMMATRIX& skyRoate)
{
	m_SkyTransform = skyRoate;
}
void SkyRender::SetSunView(const XMMATRIX& sunView)
{
	float k = m_sunRadius / m_skySphereRadius;
	m_SunView = XMMatrixScaling(k, k, k) * sunView;
}

void SkyRender::SetSunInfo(SkyEffect& skyEffect, float r, const XMFLOAT3& color)
{
	m_sunRadius = r;
	skyEffect.SetSunColor(color);
}
...
{
	HRESULT hr;
	auto sphere = Geometry::CreateSphere<VertexPos>(skySphereRadius,40,40);
	m_skySphereRadius = skySphereRadius;
